 ...
 * Transforms crypto_markets DB rows to PredictionMarket frontend format.
 * Supports binary (Up/Down, Yes/No, custom labels) and multi-outcome markets.
 * Kalshi prices are always null for crypto markets.
 */

export interface CryptoMarketOutcome {
  label: string;
...
export interface PredictionMarketFrontend {
  id: string;
  slug: string;
  question: string;
  category: 'crypto';
...
/** Parse a raw price value (string or number, 0-1 scale) to a percentage (0-100). */
...
/** Safe array extraction from potentially non-array JSONB values. */
function safeArray<T>(val: unknown): T[] {
  return Array.isArray(val) ? val : [];
}

/**
 * Extract a human-readable label from a sub-market question.
 * e.g. "Will Bitcoin reach $150,000 in February?" → "$150,000"
 * e.g. "Will Bitcoin dip to $60,000 in February?" → "$60,000 (dip)"
 */
...
/**
 * Extract short, differentiating labels for multi-outcome sub-market questions.
 * Finds the longest common prefix and suffix among all questions, then extracts
 * the unique middle portion as the label.
 * e.g. ["MicroStrategy sells any Bitcoin in 2025?",
 *        "MicroStrategy sells any Bitcoin by December 31, 2026?"]
 *    → ["in 2025", "by December 31, 2026"]
 */
function extractDifferentiatingLabels(questions: string[]): string[] {
  ...
  // Snap prefix back to last word boundary (space or start) to avoid splitting words
  while (prefixLen > 0 && shortest[prefixLen - 1] !== ' ') {
    prefixLen--;
  }

  // Find longest common suffix (character-level)
  let suffixLen = 0;
  ...
  // Snap suffix forward to next word boundary
  ...
  return questions.map((q) => {
    ...
    // Clean trailing/leading punctuation and strip leading prepositions (by, in, to, on)
...
/**
 * Build outcomes for a single sub-market (binary: 2 outcomes).
 * Labels come directly from the outcomes array (Up, Down, Yes, No, $60k, $80k, etc.)
 */
function buildBinaryOutcomes(market: SubMarket): CryptoMarketOutcome[] {
...
/**
 * Build outcomes for multi-sub-market events (e.g. "What price will BTC hit").
 * Each sub-market has Yes/No outcomes — we take the Yes price as the probability
 * and derive the label from the sub-market question.
 *
 * For price-based markets (questions contain $amounts), uses extractPriceLabel.
 * For other markets (e.g. date-based), extracts the differentiating part of
 * each sub-market question so labels don't redundantly repeat the main title.
 */
function buildMultiOutcomes(markets: SubMarket[]): CryptoMarketOutcome[] {
  const questions = markets.map((m) => m.question ?? '');

  // Check if any question contains a price — if so, use price extraction
  ...
  return markets.map((m, i) => {
    ...
    // Find the "Yes" outcome price; fall back to first price
...
export function transformCryptoMarketToFrontend(row: CryptoMarketRow): PredictionMarketFrontend {
  ...
  const subcategory = row.asset
    ? row.asset.charAt(0).toUpperCase() + row.asset.slice(1).toLowerCase()
    : 'Crypto';
